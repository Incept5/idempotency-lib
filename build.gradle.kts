...
tasks.register("publishJitPackModules") {
    dependsOn(":messaging-core:publishToMavenLocal", ":messaging-quarkus:publishToMavenLocal")
}
...
publishing {
    publications {
        create<MavenPublication>("mavenRoot") {
            ...
            // POM information
            pom {
                name.set("idempotency Library")
                description.set("idempotency  library for Quarkus applications")
                ...
                withXml {
                    ...
                    project(":idempotency-core").let { _ ->
                        val dependency = dependencies.appendNode("dependency")
                        dependency.appendNode("groupId", "com.github.incept5.idempotency-lib")
                        dependency.appendNode("artifactId", "idempotency-core")
                        dependency.appendNode("version", project.version)
                        dependency.appendNode("scope", "compile")
                    }

                    project(":idempotency-quarkus").let { _ ->
                        val dependency = dependencies.appendNode("dependency")
                        dependency.appendNode("groupId", "com.github.incept5.idempotency-lib")
                        dependency.appendNode("artifactId", "idempotency-quarkus")
                        dependency.appendNode("version", project.version)
                        dependency.appendNode("scope", "compile")
                    }

                    project(":test-quarkus-idempotency").let { _ ->
                        val dependency = dependencies.appendNode("dependency")
                        dependency.appendNode("groupId", "com.github.incept5.idempotency-lib")
                        dependency.appendNode("artifactId", "test-quarkus-idempotency")
                        dependency.appendNode("version", project.version)
                        dependency.appendNode("scope", "compile")
                    }
                }
